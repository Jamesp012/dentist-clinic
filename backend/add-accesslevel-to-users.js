...
async function addAccessLevelToUsers() {
  ...
  try {
    ...
    `);
    console.log('✓ accessLevel column added successfully to users table');
    
    // Verify
    const [columns] = await connection.query('DESCRIBE users');
    const accessLevelExists = columns.some(col => col.Field === 'accessLevel');
    if (accessLevelExists) {
      console.log('✓ Column verified in users table structure');
    }
  } catch (error) {
